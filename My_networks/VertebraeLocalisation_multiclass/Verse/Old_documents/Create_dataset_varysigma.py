...
class LoadData(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if dim3 > 128:
            indices_to_remove = []
            for i, ctd in enumerate(ctd_list[1:]):
                if not bottom_voxel <= ctd[3] <= top_voxel:
                    indices_to_remove.append(i+1)
            #Remove indices
            ctd_cropped = [i for j, i in enumerate(ctd_list) if j not in indices_to_remove]
        else:
            ctd_cropped = ctd_list
        ...
        return img, ctd_cropped, subject
