...
class LocalAppearance(nn.Module):
    def __init__(self, dropout):
        ...
        #Output
        self.output = nn.Conv3d(in_channels=64,out_channels=8,kernel_size=3, stride = 1, padding = 1)

        nn.init.normal_(self.output.weight, std=0.1)

    def forward(self, image):
        #Contracting
        layer1_skip = self.conv_down1(image)
        x = self.avgpool(layer1_skip)
        x = nn.Dropout3d(p=self.dropout)(x)
...
class SpatialConfiguration(nn.Module):
    def __init__(self,dropout):
        ...
        self.conv1 = nn.Conv3d(in_channels=8, out_channels=64, kernel_size=7, stride = 1, padding = padding)
        ...
        self.conv4 = nn.Conv3d(in_channels=64, out_channels=8, kernel_size=7, stride = 1, padding = padding)

        nn.init.normal_(self.conv4.weight, std=0.1)
...
class VertebraeLocalisationNet(nn.Module): #SpatialConfigurationNet
    def __init__(self,dropout):
        ...
        self.SpatialConfiguration = SpatialConfiguration(dropout)
